...
export const listProject = async (projectCode: string): Promise<IssueFull[]> => {
  return await multipleResult(
    () =>
      db.query<[IssueDBFull[]]>("SELECT id, in.* AS issue FROM issue_code:[$project, none]..[$project];", {
        project: projectCode,
      }),
  );
};
...
export const create = async (projectCode: string, issue: Issue): Promise<IssueFull> => {
  ...
  const lastCode = (await db.query<[{ id: [string, number] } | undefined]>(
    "SELECT id FROM ONLY issue_code:[$project, none]..[$project] ORDER BY id DESC LIMIT 1",
    { project: projectCode },
  ))[0];
  ...
  return {
    code: `${projectCode}-${nextNumber}`,
    state: issueDB[0].state.id,
    last_modified: issueDB[0].last_modified,
    title: issueDB[0].title,
    summary: issueDB[0].summary,
    description: issueDB[0].description,
    data: issueDB[0].data,
  };
};
