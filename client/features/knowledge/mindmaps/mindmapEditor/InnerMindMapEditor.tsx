...
import { MindMap, MindMapConnection, MindMapNode as MindMapNodeType } from "~/types/knowledge";
...
export function InnerMindMapEditor({
  ...
  const handleFinishEditNodeName = useCallback(() => {
    if (!selectedNode) return;
    setNodes((ns) =>
      ns.map((n) => n.id === selectedNode.id ? { ...n, data: { ...n.data, label: newNodeName || n.data.label } } : n)
    );
    ...
    <div className="flex flex-col h-full border rounded-lg overflow-hidden">
      ...
      {selectedNode && !readOnly && (
        <NodePanel
          ...
          onChangeColor={(color) => {
            setNodes((ns) => ns.map((n) => n.id === selectedNode.id ? { ...n, data: { ...n.data, color } } : n));
            setHasUnsavedChanges(true);
            setColorPopoverOpen(false);
          }}
          ...
          onChangeShape={(shape) => {
            setNodes((ns) => ns.map((n) => n.id === selectedNode.id ? { ...n, data: { ...n.data, shape } } : n));
            setHasUnsavedChanges(true);
            setShapePopoverOpen(false);
          }}
