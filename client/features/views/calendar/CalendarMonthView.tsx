...
export function CalendarMonthView({ days, issues, onSelectDay, onIssueSelect }: MonthViewProps) {
  ...
  const rows = Array.from({ length: totalRows }, (_, i) => days.slice(i * 7, (i + 1) * 7));
  const lastRowHasCurrentMonth = rows[rows.length - 1].some((day) => day.isCurrentMonth);

  return (
    <>
      <div className="ring-1 shadow-sm ring-border rounded-lg overflow-hidden lg:flex lg:flex-auto lg:flex-col">
          ...
          <div className="px-4 py-6 sm:px-6 lg:hidden">
            ...
            <ol className="divide-y divide-border overflow-hidden rounded-lg bg-card text-sm shadow-sm">
              {selectedDay.events.map((event) => (
                <li key={event.id} className="flex p-4 pr-6">
                  <div className="flex-auto">
                    <p className="font-semibold text-foreground">{event.name}</p>
                    <time dateTime={event.datetime} className="mt-2 flex items-center text-muted-foreground">
          ...
          <div className="px-4 py-6 sm:px-6 lg:hidden">
            {selectedDay && (
              <div className="text-center p-4 text-muted-foreground">
                <p>
                  Žádné události {new Date(selectedDay.date).toLocaleDateString("cs-CZ", {
                    day: "numeric",
                    month: "long",
                  })}
                </p>
