...
import { mdxToDocument, generateUniqueSlug, getMdxFiles, getMdxDocumentBySlug } from '~/utils/knowledge/mdxUtils';
...
vi.mock('fs', () => {
  ...
  const mockFs = {
    ...
    writeFileSync: vi.fn(),
    mkdirSync: vi.fn(),
    __setMockFiles: (f: string[]) => { files = f; },
    __setMockContent: (c: string) => { fileContent = c; },
    __setExists: (e: boolean) => { exists = e; }
  };
