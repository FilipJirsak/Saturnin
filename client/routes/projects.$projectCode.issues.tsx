...
import { ArrowUpDown, Check, Edit, Eye, Filter, Search, X } from "lucide-react";
import { useMemo, useState } from "react";
import { ISSUE_STATES, ISSUE_TEAM_MEMBERS } from "~/lib/constants";
import { Popover, PopoverContent, PopoverTrigger } from "~/components/ui/popover";
import { Label } from "~/components/ui/label";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "~/components/ui/select";
import { filterAndSortIssues, getStateColorClasses, getStateLabel, hasActiveFilters } from "~/utils/issueUtils";
    ...
    <Card className="bg-card text-card-foreground">
            ...
            <PopoverTrigger asChild>
              <Button
                ...
                className={`gap-2 transition-all ${
                  areFiltersActive ? "bg-primary text-primary-foreground hover:bg-primary/90" : ""
                }`}
              >
                ...
                <div className="grid gap-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setTempFilters((prev) => ({ ...prev, state: value === "all" ? null : value }))}
                    >
                      <SelectTrigger
                        id="state"
                        className="w-full bg-background border-input focus:ring-1 focus:ring-ring"
                      >
                        ...
                        {ISSUE_STATES.map((state) => {
                          ...
                          return (
                            <SelectItem key={state.value} value={state.value} className="focus:bg-muted">
                              <div className="flex items-center gap-2">
                                <div className={`w-3 h-3 rounded-full flex-shrink-0 border ${stateColors.circle}`}>
                                </div>
                                {state.label}
                              </div>
                            </SelectItem>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setTempFilters((prev) => ({ ...prev, assignee: value === "all" ? null : value }))}
                    >
                      <SelectTrigger
                        id="assignee"
                        className="w-full bg-background border-input focus:ring-1 focus:ring-ring"
      ...
      <CardContent>
        <div className="rounded-md border border-border">
          <Table>
            <TableCaption className="pr-4 pb-4 italic text-end">
              Celkem: {filteredAndSortedIssues.length} issues
            </TableCaption>
            <TableHeader>
              <TableRow>
