...
const mdxComponents = {
  ...
  blockquote: (props: any) => <blockquote className="border-l-4 border-muted-foreground pl-4 italic my-3" {...props} />,
  a: ({ href, ...props }: any) => {
...
function SimpleMarkdownRenderer({ content }: { content: string }) {
  const processedContent = useMemo(() => {
    ...
    const withoutJsxComponents = cleanContent
      .replace(
        /<Card.*?<\/Card>/gs,
        '<div class="p-4 my-4 border rounded-md bg-muted/20"><div class="font-bold">Důležité upozornění</div><div>Toto je interaktivní komponenta (nedostupná v jednoduchém zobrazení)</div></div>',
      )
      .replace(
        /<Badge.*?>(.*?)<\/Badge>/g,
        '<span class="inline-block px-2 py-1 text-xs rounded-full bg-muted text-foreground">$1</span>',
      )
      .replace(
        /<Button.*?>(.*?)<\/Button>/g,
        '<span class="inline-block px-3 py-1 text-sm rounded-md bg-primary text-primary-foreground">$1</span>',
      )
    ...
    const html = withoutJsxComponents
      ...
      .replace(
        /\`\`\`([\s\S]*?)\`\`\`/g,
        (match, code) => `<pre class="bg-muted p-3 rounded-md overflow-x-auto my-4"><code>${code}</code></pre>`,
      )
